...
namespace Inferno {
    ...
    void ResetGamepadBindings(InputDeviceBinding& device, float deadzone) {
        ...
        // Shoulders
        device.Bind({ .action = GameAction::RollLeft, .id = SDL_GAMEPAD_BUTTON_LEFT_SHOULDER, .type = BindType::Button });
        device.Bind({ .action = GameAction::RollRight, .id = SDL_GAMEPAD_BUTTON_RIGHT_SHOULDER, .type = BindType::Button });

        // Dpad bindings
        device.Bind({ .action = GameAction::Headlight,      .id = SDL_GAMEPAD_BUTTON_DPAD_UP,    .type = BindType::Button });
        device.Bind({ .action = GameAction::CycleBomb,      .id = SDL_GAMEPAD_BUTTON_DPAD_DOWN,  .type = BindType::Button });
        device.Bind({ .action = GameAction::CycleSecondary, .id = SDL_GAMEPAD_BUTTON_DPAD_LEFT,  .type = BindType::Button });
        device.Bind({ .action = GameAction::CyclePrimary,   .id = SDL_GAMEPAD_BUTTON_DPAD_RIGHT, .type = BindType::Button });
#endif
    }
}
